...
def test_exceptions_outside_the_list_are_not_retried():
    mock_func = MagicMock(side_effect=KeyError("config"))

    decorator = exponential_backoff(max_retries=3, initial_delay=0.01, exceptions=(TimeoutError,))
    decorated_func = decorator(mock_func)
